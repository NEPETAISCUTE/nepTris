...
void dbgSetBoard(Board b, const char *map, Vec2D offset, Vec2D size) {
  for (int8_t y = 0; y < size.y; y++) {
    for (int8_t x = 0; x < size.x; x++) {
      b[y + offset.y][x + offset.x] =
          map[y * BOARD_WIDTH + x] == 'X' ? PIECE_I : PIECE_NONE;
    }
  }
}
...
Game *gameInit() {
  ...
  g->colorChangeSupport = can_change_color() && COLORS >= 8 && COLOR_PAIRS >= 8;
  if (g->colorSupport && g->colorChangeSupport && COLORS >= 8 &&
      COLOR_PAIRS >= 8) {
    saveColors(g->tclr);
    setColors(gamePltt);
  ...
  g->scoreWin =
      newwin(SCOREWIN_HEIGHT, SCOREWIN_WIDTH, 0, (BOARD_WIDTH * 2) + 2);
  if (!g->scoreWin) {
    gameQuit(g, false, -1);
    return NULL;
  }
...
void gameQuit(Game *g, bool quit, int errcode) {
  if (g != NULL) {
    if (has_colors() && can_change_color() && COLORS > 8 && COLOR_PAIRS > 8)
      setColors(g->tclr);
    if (g->boardWin)
      delwin(g->boardWin);
    if (g->scoreWin)
...
void drawGameInfo(Game *g) {
  ...
  mvprintw(26, 0,
           "press %c to rotate the piece counterclockwise and %c to rotate it "
           "clockwise",
           GK_ROTATE_CCW, GK_ROTATE_CLW);
  mvprintw(27, 0,
           "press %c to hold the piece (second slot on the small window at the "
           "right of the board)",
           GK_HOLD);
  mvprintw(28, 0, "press %c to toggle chicken mode : %s", GK_CHICKEN,
           g->chickenMode ? "enabled " : "disabled");
  mvprintw(29, 0,
           "press %c to pause/unpause, %c to quit, and %c to reset the game",
           GK_PAUSE, GK_QUIT, GK_RESET);
...
void runStateStart(Game *g) {
  ...
  mvwprintw(g->boardWin, BOARD_HEIGHT / 2 + 1,
            BOARD_WIDTH + 1 - (getDigitCnt(g->highScore) / 2), "%lu",
            g->highScore);
  if (!g->colorSupport)
    mvwprintw(g->boardWin, BOARD_HEIGHT / 2 + 2, BOARD_WIDTH - 5,
              "your terminal\n doesn't support\n colors");
  else if (!g->colorChangeSupport)
    mvwprintw(g->boardWin, BOARD_HEIGHT / 2 + 2, BOARD_WIDTH - 5,
              "your terminal\n doesn't support\n color editing\n or does not "
              "have enough colors \n or color pairs");
...
void runStateGame(Game *g) {
  ...
  if (!g->isAnim) {
    ...
    if (g->input == GK_ROTATE_CLW || g->input == GK_ROTATE_CCW)
      rotatePiece(g);

    if (g->input == GK_HOLD || g->holdBuffer)
      swapPiece(g);
    g->holdBuffer = false;

    if (g->input == GK_G_UP)
      g->gravity++;
    if (g->input == GK_G_DOWN)
      g->gravity--;

    if (g->input == GK_LEFT)
      g->piece.newPos.x = g->piece.pos.x - 1;
    if (g->input == GK_RIGHT)
      g->piece.newPos.x = g->piece.pos.x + 1;
    ...
    if (g->gravity != g->lastGravity) {
      ...
      if (g->gravity > G_MAX_DROP) {
        if (g->maxLockTimer > 0)
          g->maxLockTimer -= 2;
        if (g->maxMoveResetCnt > 0)
          g->maxMoveResetCnt--;
      }
    }
    if (g->input == GK_HARD_DROP)
      hardDrop(g);
    updateVertPos(g);
    if (g->isAnim)
      return;
    ...
    if (g->chickenMode)
      drawTetraminoShadow(g->boardWin, &g->piece);
    ...
    if (g->isAllClear)
      mvwprintw(g->boardWin, BOARD_HEIGHT / 2, BOARD_WIDTH - 4, "All Clear");
    g->animTimer--;
    if (g->animTimer == 0)
      g->isAnim = false;
  }
  if (g->specialActName != NULL) {
    mvwprintw(g->scoreWin, 16, 1, "%s %s", g->specialActName,
              (g->lastDifficult && g->currentDifficult && g->isDifficult)
                  ? "B2B"
                  : "");
    g->specialActTimer--;
  ...
  if (g->combo > 1)
    mvwprintw(g->scoreWin, 17, 1, "Combo X %lu", g->combo);
...
void runStateOver(Game *g) {
  ...
  mvwprintw(g->boardWin, BOARD_HEIGHT / 2 + 3,
            BOARD_WIDTH + 1 - (getDigitCnt(g->score) / 2), "%lu", g->score);
  if (g->isHighScore)
    mvwprintw(g->boardWin, BOARD_HEIGHT / 2 + 4, BOARD_WIDTH - 4, "Hi Score!");
  wattroff(g->boardWin, A_BOLD);
  if (g->input == GK_START) {
    gameReset(g);
    g->state = GSTATE_START;
...
void checkGameover(Game *g) {
  if (collideBoard(g->board, g->piece.ID, g->piece.newRot, g->piece.newPos.x,
                   g->piece.newPos.y))
    g->state = GSTATE_OVER;
}
...
void rotatePiece(Game *g) {
  if (g->input == GK_ROTATE_CLW)
    g->piece.newRot = (g->piece.rot + 1) & ROT_MAX;
  else
    g->piece.newRot = (g->piece.rot - 1) & ROT_MAX;
}

void softDrop(Game *g, bool gravity) {
  if (g->input == KEY_DOWN && !g->isOnGround)
    g->score++;
...
void calcHardDropPos(Game *g) {
  ...
  while (!collideBoard(g->board, g->piece.ID, g->piece.rot, g->piece.pos.x,
                       g->piece.hardDropy) &&
...
void updateHorzPos(Game *g) {
  if (g->piece.pos.x == g->piece.newPos.x && g->piece.rot == g->piece.newRot)
    return;
  ...
  if (g->piece.pos.x != g->piece.newPos.x) {
    ...
    if (!collideHorzBorders(g->piece.ID, g->piece.newRot, g->piece.newPos.x) &&
        !collideBoard(g->board, g->piece.ID, g->piece.newRot, g->piece.newPos.x,
                      g->piece.pos.y)) {
    ...
    isMovementPossible = handleRot(g);
  if (isMovementPossible && g->isOnGround && g->moveResetCnt > 0) {
    g->moveResetCnt--;
    g->isOnGround =
        collideVertBorders(g->piece.ID, g->piece.rot, g->piece.pos.y + 1) ||
        collideBoard(g->board, g->piece.ID, g->piece.rot, g->piece.pos.x,
                     g->piece.pos.y + 1);
    g->lockTimer = 30;
  }
}
...
bool handleRot(Game *g) {
  ...
  if (g->piece.ID == PIECE_I)
    kicks = &kickTableI;
  bool isCCL = (g->piece.rot == 0 && g->piece.newRot == 3) ||
               (g->piece.newRot < g->piece.rot &&
                !(g->piece.rot == 3 && g->piece.newRot == 0));
  for (int i = 0; i < 5; i++) {
    ...
    if (!collideHorzBorders(g->piece.ID, g->piece.newRot,
                            g->piece.newPos.x + pos.x) &&
        !collideBoard(g->board, g->piece.ID, g->piece.newRot,
                      g->piece.newPos.x + pos.x, g->piece.newPos.y + pos.y) &&
        !collideVertBorders(g->piece.ID, g->piece.newRot,
                            g->piece.newPos.y + pos.y)) {
      g->piece.pos.x = g->piece.newPos.x + pos.x;
      g->piece.newPos.x = g->piece.pos.x;
      g->piece.rot = g->piece.newRot;
      g->piece.pos.y = g->piece.newPos.y + pos.y;
      g->piece.newPos.y = g->piece.pos.y;
      if (g->piece.ID == PIECE_T)
        g->tspin = checkTSpin(g->board, g->piece.rot, g->piece.pos.x,
                              g->piece.newPos.y);
      return true;
    }
  }
...
void updateVertPos(Game *g) {
  if (g->piece.pos.y == g->piece.newPos.y && !g->isOnGround)
    return;
  ...
  placeTiles(g->board, g->piece.ID, g->piece.rot, g->piece.pos.x,
             g->piece.pos.y);
  g->isAllClear = false;
  ...
  case 4:
    ...
    g->isDifficult = true;
    if (g->isAllClear) {
      if (g->lastDifficult && g->currentDifficult)
        g->scoreAdd += 3200 * g->gravity;
      else
        g->scoreAdd += 2000 * g->gravity;
    }
    break;
  default:
    if (g->tspin == TSPIN_REGULAR) {
  ...
  if (g->scoreAdd - lastScoreAdd > 0)
    g->scoreAddTimer = 60;
  if (g->specialActName != NULL)
    g->specialActTimer = 120;
  if (lnCnt > 0)
    g->combo++;
  else
    g->combo = 0;
...
void getNewPiece(Game *g) {
  if (collideBoard(g->board, g->nextPiece, GAME_SPAWNROT, GAME_SPAWNX,
                   GAME_SPAWNY)) {
    g->state = GSTATE_OVER;
    return;
  }
...
int handleLines(Game *g) {
  ...
  for (int i = len - 1; i >= 0; i--) {
    memmove(&g->board[top + 1], &g->board[top],
            ((lineIdx[i] - top) * BOARD_WIDTH * sizeof(Piece)));
